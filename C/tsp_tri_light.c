...
// DXP Crossover
void dxp_crossover(int* parent_a, int* parent_b, int* child, int dimension, int (*distance)(Node, Node), Node* nodes,FichierTSP * tsp) {
    // 1. Copier parent A
    ...
    for (int gen = 0; gen < GENERATIONS; gen++) {
        ...
        for (int i = 0; i < POPULATION_SIZE; i += 2) {
            if (i + 1 < POPULATION_SIZE) {
                if(dxp){
                    dxp_crossover(selected[i], selected[i+1], offspring[i], dimension,distance,tsp->nodes,tsp);
                    dxp_crossover(selected[i+1], selected[i], offspring[i+1], dimension,distance,tsp->nodes,tsp);
                }else{
